...
router.addBooking = (req, res) => {
    ...
    if (booking.name == null || booking.email == null || booking.contactNum == null || booking.quantity == null || booking.roomType == null || booking.checkin_date == null || booking.leave_date == null) {
...
router.success = (req, res) => {
    ...
    //
    paypal.payment.execute(paymentId, execute_payment_json, function (error, paymentLog) {
        if (error) {
            console.log(error.response);
            throw error;
        } else {
            console.log("Get Payment Response");
            console.log(JSON.stringify(paymentLog,null,5));
    //             // Booking.findOneAndUpdate({_id: req.params._id},
    //             //     {
    //             //         payment_status: 'paid'
    //             //     },
